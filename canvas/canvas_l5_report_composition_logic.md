...
- 可依產業類型（`industry_type`）自動呈現不同的內容模版（如：冷鏈物流、廚房設備、IT 設備等）

### 📄 制度型報告（SOP 附件建議）

- 依據問卷中明確回答的制度問題，自動產出「制度建議草案」
- 含表單格式建議、部門責任分工、紀錄流程與稽核頻率（參考附錄 SOP 樣式）
...
📋 建議紀錄方式：
- 每月記錄冷藏庫電表讀數，並保留帳單掃描件
- 每週統整外送騎士加油紀錄，由管理員定期輸入 Google Sheet
```
...
| `industry_type`     | 控制語氣脈絡與段落主題樣板，並帶入該產業對應 SOP 建議                   |
...
- ✅ SOP 類題可產出表格格式建議，並分部門對應記錄責任
- 📌 未來可支援報告語氣客製化（保守 / 積極 / 咨詢式）
- 📊 報告可輸出 HTML / PDF / JSON 結構（適用不同前端）

---

> 此 Canvas 為報告模組實作基礎，亦可導出為報告模板引擎規格、自然語言組合模板、SOP 協助產出模組、知識庫對接邏輯等。
